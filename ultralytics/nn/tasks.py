...
try:
    # 确保项目根目录在 sys.path 中
    # 假设 tasks.py 在 ultralytics/nn/ 下，根目录在 ../../
    ...
    # 1. 将 modules 里的所有类导入到当前命名空间，以便 eval(字符串) 能找到它们
    # 相当于执行了 from modules import *
    for name, obj in inspect.getmembers(modules):
        if inspect.isclass(obj) and issubclass(obj, nn.Module):
            globals()[name] = obj

    # 2. 生成一个列表，存下所有自定义模块的类，给 parse_model 用
    CUSTOM_MODULES = tuple(
        obj for name, obj in inspect.getmembers(modules)
        if inspect.isclass(obj) and issubclass(obj, nn.Module)
    )
    print(f"成功加载自定义模块: {[m.__name__ for m in CUSTOM_MODULES]}")

except ImportError as e:
    print(f"未找到自定义模块文件夹 'modules' 或加载失败: {e}")
    CUSTOM_MODULES = ()
